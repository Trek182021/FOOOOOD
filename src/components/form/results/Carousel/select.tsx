...
const CarouselSelect = ({ selectedFood, handleFoodChange, currentData }: CarouselSelectProps) => {
    ...
    useEffect(() => {
        if (currentData && selectedFood) {
            const matchingFood = currentData.instances.find(food => food.description === selectedFood);
            const { calories, carbohydrates, protein } = matchingFood!.nutritionalFacts;
                    ...
                    <SelectLabel className="font-nutrition">Food Options</SelectLabel>
                        {
                            currentData['instances'].map((foodItem) => (
                                <SelectItem key={foodItem.description} value={foodItem.description} className="font-nutrition font-inherit">{foodItem.description}</SelectItem>
                            ))
                        }
                    </SelectGroup>
